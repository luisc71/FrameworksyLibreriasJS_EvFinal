...
// *** Funciones para animar cambio de color del título ***
function colorUno(){
  $("h1").animate({color: '#dcff0e'}, 500, function(){
    colorDos();
  });
}
function colorDos(){
  $("h1").animate({color: '#ffffff'}, 500, function(){
    colorUno();
  });
}
...
function desplazamiento(){
  ...
  if(i==8){
    clearInterval(intervalo);   //Desactivar función desplazamiento()
    eliminar=setInterval(function(){eliminarHorver()},150)  //Activar función eliminarHorver()
  }
}

// *** Función contador a cero ***
function timer(){
  if(seg!=0){
    seg=seg-1;
  }
...
// *** Función eliminar más de 3 dulces ***
function eliminarHorver(){
  matriz=0;
  rbh=horizontal()  //función buscar dulces horizontal
  rbv=vertical()    //función buscar dulces vertical

  for(var j=1;j<8;j++){
      matriz=matriz+$(".col-"+j).children().length;
  }
  // *** Condición si no encuentra 3 dulces o más llamar a función para volver a completar el juego ***
  if(rbh==0 && rbv==0 && matriz!=49){
      clearInterval(eliminar);
      bnewd=0;
      newdulces=setInterval(function(){
        nuevosdulces()  //Función completar nuevos dulces
      },600)
  }
  if(rbh==1 || rbv==1){
    $(".elemento").draggable({ disabled: true });
    ...
    $(".activo").hide("pulsate",1000,function(){
      ...
      $("#score-text").html(score)  //Cambiar puntuación
    })
  }
...
// *** Función búsqueda horizontal de dulces ***
function horizontal(){
  var bh=0;
  for(var j=1;j<8;j++){
    for(var k=1;k<6;k++){
...
// *** Función búsqueda vertical de dulces ***
function vertical(){
  var bv=0;
  for(var l=1;l<6;l++){
    for(var k=1;k<8;k++){
